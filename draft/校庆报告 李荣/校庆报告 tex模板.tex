...
本文发现训练NTM学习加法，乘法等算法时，模型性能较差，因此本文使用了Bengio提出的普通的课程学习策略\upcite{bengio2009curriculum}和Zaremba\upcite{zaremba2014learning}提出的混合课程学习策略效果训练模型，然而这两种课程学习策略并没有取得理想的效果。因此，本文提出了一种新的课程学习策略，改善了模型的性能并且加快了模型的收敛速度。

最后，本文发现选择适合的记忆模块的大小，有助于提升模型的性能。可以通过复制例如：在复制算法（3.1节将具体介绍该任务）中，复制长度为20 的字符序列，理论上只有20个记忆单元（每个记忆单元存储一个字符）。如果记忆单元的个数如果少了，则模型的性能会下降，反之，多余的记忆单元则不会影响模型的性能（多余的记忆单元可以不使用）。
